...
async def test_aerospike_happy(service_client, sentinel_gate, gate):
...
@pytest.mark.skip(reason='Flaky test TAXICOMMON-6075')
async def test_aerospike_disable_reads(service_client, sentinel_gate, gate):
    gate.to_server_noop()
...
async def test_aerospike_disable_writes(service_client, sentinel_gate, gate):
    gate.to_client_noop()
    result = await service_client.delete('/chaos?key=foo')
    assert result.status == 500
    gate.to_client_pass()
...
async def test_aerospike_close_connections(service_client, sentinel_gate, gate):
